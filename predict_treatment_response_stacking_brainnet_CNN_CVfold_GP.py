...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 22 12:44:06 2020
...
#      print  
    ...
    dists = pdist(train_stacking_vars)
    #k = cov.RBF(log_ell = np.log(np.median(dists)))
    n_features = np.shape(train_stacking_vars)[1]
    k = cov.RBFard(log_ell_list = [np.log(np.median(dists))] * n_features)    
    # add priors
    rgr.setPrior(kernel=k, mean=m)
    
    # set likelhood sd
    rgr.setNoise( log_sigma = np.log(np.std(train_targets)))
    
    print ('hyps pre-optimization:')
    print (rgr.covfunc.hyp)
    print (rgr.meanfunc.hyp)
    print (rgr.likfunc.hyp)
    
    # optimise the hyperparameters
    rgr.optimize(train_stacking_vars, train_targets, numIterations=200)
    
    print ('hyps post-optimization:')
    print (rgr.covfunc.hyp)
    print (rgr.meanfunc.hyp)
    print (rgr.likfunc.hyp)
    
    # make predictions on training data
    ym, ys2, fm, fs2, lp = rgr.predict(test_stacking_vars)
        
    # store results
    predicted_followup_total_PANSS[start_ind:stop_ind, 0] = np.squeeze(ym)
...
# save results DF
subjects_response_results.to_csv(results_dir + 'predict_treatment_response_stacked_results_brainnet_CNN_MAE_GP_RBF_ARD_' + data_type + '_' + network_size + '_' + site_correction + '_CV_fold_' + job_id + '.csv')