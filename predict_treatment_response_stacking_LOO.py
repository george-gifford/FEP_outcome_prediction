...
predicted_followup_total_PANSS = np.zeros((n_subjects, 1))
...
train_size = n_subjects - 1
for i, (train_index, test_index) in enumerate(loo_outer.split(K)) :
...
#        
    ...
    # store results
    predicted_followup_total_PANSS[test_index, 0] = final_preds
    test_baseline_total_PANSS[test_index, 0] = baseline_total_PANSS[test_index]
    test_followup_total_PANSS[test_index, 0] = followup_total_PANSS[test_index]
...
# save raw results tabe;
subjects_response_results.to_csv(results_dir + 'LOO_results_' + site_correction + '.csv')
